...
class Settings(BaseSettings):
    ...
    # Gateway de embeddings/geração/reranking LiteLLM (RAG-025/RAG-030/
    # RAG-033) — provisionado pelo docker-compose.yml (serviços
    # `litellm`+`ollama`, RAG-030). `litellm_api_key` é opcional porque
    # um proxy local de desenvolvimento pode não exigir autenticação;
    # nunca deixar de configurá-lo em production.
    ...
    # RAG-033 — "pode ser desativado" (critério de aceite): qual
    # RerankerPort é injetado (LiteLLMReranker vs. PassthroughReranker)
    # depende deste flag, não de um `if` dentro de um adapter só. Default
